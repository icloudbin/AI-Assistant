...
## OpenRouter support: Ox Alpha (v1.10.0)

The model dropdown now has a fifth entry, "OpenRouter: Ox Alpha," and Settings has a fifth, independent "OpenRouter API Key" field.

This one is different in kind from the other four: OpenRouter (https://openrouter.ai) is a third-party router/aggregator, not a model's own native API - it exposes an OpenAI-Chat-Completions-compatible endpoint that forwards to whichever underlying provider/model a slug names. "Ox Alpha" (`stealth/ox-alpha`) is a stealth/anonymous model OpenRouter itself listed on 2026-08-20: the underlying provider has chosen to stay anonymous during this preview, OpenRouter is only routing to it and is not its developer or owner, and per OpenRouter's own stealth-model page, pricing is currently $0/$0 but explicitly preview-only (could change or the model could disappear without notice), and prompts/completions are retained by that anonymous provider - not used for training, but a different data-handling posture than calling Anthropic/OpenAI/Google directly under their own terms. None of that is enforced or worked around in code; it's just worth knowing before pointing anything sensitive at it. (Multiple outlets have speculated about who's actually behind it - Z.ai's GLM family and an unreleased Microsoft MAI model have both been floated - but none of that is confirmed, so none of it is repeated here as fact.)

Implementation notes:
- `background.js` gained a fifth branch, `streamOpenRouter`, at `https://openrouter.ai/api/v1/chat/completions` (confirmed against OpenRouter's own quickstart docs). Unlike Claude and ChatGPT, OpenRouter's request shape is the OLDER OpenAI Chat-Completions format - the same `{messages: [{role, content}]}` shape DeepSeek's API already uses, system role sent inline rather than a separate top-level field - so `streamOpenRouter` reuses `buildMessages()` and the exact same streaming-extraction shape as `streamDeepSeek` (`choices[0].delta.content`), just pointed at a different host/key/model slug. Two optional attribution headers (`HTTP-Referer`, `X-OpenRouter-Title`) are included, per OpenRouter's docs, so requests from this extension are identifiable on OpenRouter's own dashboard rather than anonymous - they don't affect whether a request works.
- **CORS again could not be fully verified** (same caveat as ChatGPT in v1.9.0, for the same sandbox network-access reason). OpenRouter's own docs show plain `fetch()`/`requests` examples aimed at being called directly from client apps, with none of the backend-proxy guidance OpenAI's docs give - so a CORS block seems less likely here than it was for OpenAI - but that's a reasonable inference from how OpenRouter presents itself, not a confirmed test result, so this still needs a real check with a live key.
